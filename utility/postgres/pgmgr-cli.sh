...
function main__HELP()
{
  echo "
------------------------------------------------------------------------------------------------
SAS INSITIUTE INC. - POSTGRESQL MANAGER COMMAND LINE INTERFACE
...
function main()
{
  ...
  local -r OPTIONS=$(getopt -o b:s::drh -l "database:,retore-file:,dump,restore,help" -n "${FUNCNAME[0]}" -- "$@") || return
  eval set -- "$OPTIONS"
  ...
  # Verify Flags
  [[ -n ${PGDATABASE} ]] || { echo "ABORTED: Database must be specified."; return 1; }
